...
function evaluateFix(fix, script, csp, contentReplace) {
    switch(fix) {
        ...
        case "reddit-comments-regexp":
            ...
            contentReplace.push([String.raw`var s;const{subreddit:o,postId:r}=(null===(s=t.match(i))||void 0===s?void 0:s.groups)||{};`, String.raw`const s=t.match(i),o=!s||!s[1]?void 0:s[1],r=!s||!s[2]?void 0:s[2];`]);
            break;
        case "tmx-optchain":
...
function supersededFixes(service) {
    ...
    // If possible, perform feature detection rather than check UXP platform version, application or its version.
    // Keep entries in each block in the same order as in evaluateFix's switch statement.
    const superseded = new Set();
    // polyfills
    if (typeof service.domWindow.Array.prototype.flat === "function") {
        superseded.add("std-Arrayflat");
    }
    if (typeof service.domWindow.Array.prototype.flatMap === "function") {
        superseded.add("std-ArrayflatMap");
    }
    if (typeof service.domWindow.customElements === "object") {
        superseded.add("std-customElements");
    }
    if (typeof service.domWindow.Element.prototype.attachShadow === "function") {
        superseded.add("std-ElementattachShadow");
        superseded.add("std-ShadowDOM");
    }
    if (typeof service.domWindow.Element.prototype.toggleAttribute === "function") {
        superseded.add("std-ElementtoggleAttribute");
    }
    if (typeof service.domWindow.Intl.RelativeTimeFormat === "function") {
        superseded.add("std-IntlRelativeTimeFormat");
    }
    if (typeof service.domWindow.PerformanceObserver === "function") {
        superseded.add("std-PerformanceObserver");
    }
    if (typeof service.domWindow.String.prototype.matchAll === "function") {
        superseded.add("std-StringmatchAll");
    }
    if (typeof service.domWindow.queueMicrotask === "function") {
        superseded.add("std-queueMicrotask");
    }
    // JS syntax
    // optional-chaining operator
    if (service.isSupportedJSSyntax("a?.b")) {
        superseded.add("dhl-optchain");
        superseded.add("gh-script-optchain");
        superseded.add("tmx-optchain");
    }
    // regexp named capture group
    if (service.isSupportedJSSyntax("/(?<a>)/")) {
...
class PolyfillService {
    constructor() {
        this.domWindow = Services.wm.getMostRecentWindow("navigator:browser");
    ...
    _onWindowLoad() {
        this.domWindow.removeEventListener("load", this._onWindowLoad, false); // this removes this load function from the window
        // window has loaded, now we can setup rules
        this._loadRules();
    ...
    _waitForWindow() {
        // no window on browser startup
        if (!this.domWindow) {
            this.windowListener = {
                that: this,
                onOpenWindow: function (aWindow) {
                    // wait for the window to finish loading
                    this.that.domWindow = aWindow.QueryInterface(Ci.nsIInterfaceRequestor).getInterface(Ci.nsIDOMWindow);
                    this.that.domWindow.addEventListener("load", this.that._onWindowLoad.bind(this.that), false);
                }
            };
